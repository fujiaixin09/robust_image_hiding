...
class Prep_pureUnet(nn.Module):
    def __init__(self,config=GlobalConfig()):
        ...
        self.downsample_8 = nn.Sequential(
            nn.Conv2d(3, 64, kernel_size=3, stride=1, dilation=1, padding=1), #1
            nn.ELU(inplace=True),
        ...
        self.downsample_5 = nn.Sequential(
            PureUpsampling(scale=1 / 2),
            SingleConv(256, out_channels=512, kernel_size=3, stride=1, dilation=1, padding=1), #1
            SingleConv(512, out_channels=512, kernel_size=3, stride=1, dilation=1, padding=1), #2
        )
        # 16
        self.downsample_4 = nn.Sequential(
            PureUpsampling(scale=1 / 2),
            SingleConv(512, out_channels=512, kernel_size=3, stride=1, dilation=1, padding=1), #1
            SingleConv(512, out_channels=512, kernel_size=3, stride=1, dilation=1, padding=1), #2
        )
        # 16以下的卷积用4层conv
        ...
        self.pureUpsamle = PureUpsampling(scale=256/128)
        self.pureUpsamle4 = PureUpsampling(scale=256/64)
        # self.downsample = PureUpsampling(scale=64/256)
        # 32
        self.upsample4_3 = nn.Sequential(
            SingleConv(1024, out_channels=512, kernel_size=3, stride=1, dilation=1, padding=1), #1
            SingleConv(512, out_channels=256, kernel_size=3, stride=1, dilation=1, padding=1), #2
        )
        # 64
