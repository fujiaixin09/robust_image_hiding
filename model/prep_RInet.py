...
class UnetInception(nn.Module):
    def __init__(self,config=GlobalConfig(), CoverF=32, WaterF=32):
        ...
        self.fullConv = nn.Sequential(
            SingleConv(CoverF*8+WaterF*2, out_channels=CoverF*8+WaterF*2, kernel_size=5, stride=1, dilation=1, padding=2),
            SingleConv(CoverF*8+WaterF*2, out_channels=CoverF*8+WaterF*2, kernel_size=5, stride=1, dilation=1, padding=2),
            SingleConv(CoverF*8+WaterF*2, out_channels=CoverF*8, kernel_size=5, stride=1, dilation=1, padding=2),
        ...
        self.upsample4_3 = nn.Sequential(
            SingleConv(CoverF*8*2+WaterF*2, out_channels=CoverF*8, kernel_size=3, stride=1, dilation=1, padding=1),
        ...
        self.upsample3_3 = nn.Sequential(
            SingleConv(CoverF*4*2+WaterF, out_channels=CoverF*4, kernel_size=3, stride=1, dilation=1, padding=1),
        ...
        self.finalCoverF = nn.Sequential(
            nn.Conv2d(CoverF, 3, kernel_size=1, padding=0),
            nn.Tanh()
        )
        # self.finalCoverF2 = nn.Sequential(
        #     nn.Conv2d(CoverF*2, 3, kernel_size=1, padding=0),
        #     nn.Tanh()
        # )
    ...
    def forward(self, cover, secret):
        ...
        # up0 = self.finalCoverF2(up1_cat)
        up1 = self.upsample1_3(up1_cat)
        up0 = self.finalCoverF(up1)
