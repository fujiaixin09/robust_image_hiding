...
import utils
from DeepSteg.hiding import HidingNetwork
from DeepSteg.prepare import PrepNetwork
from DeepSteg.reveal import RevealNetwork
                ...
                if idx % 128 == 127:
                    for i in range(marked.shape[0]):
                        utils.save_images(watermarked_images=marked[i].cpu(),
                                          filename='./DeepSteg/marked/epoch-{0}-marked-batch-{1}-{2}-{3}.bmp'.format(epoch, idx, i, name),
                                         std=self.config.std,
                                         mean=self.config.mean)
                        utils.save_images(watermarked_images=extracted[i].cpu(),
                                          filename='./DeepSteg/extracted/epoch-{0}-extracted-batch-{1}-{2}-{3}.bmp'.format(epoch, idx, i, name),
                                          std=self.config.std,
                                          mean=self.config.mean)
                        utils.save_images(watermarked_images=self.train_water[i].cpu(),
                                          filename='./DeepSteg/water/epoch-{0}-water-batch-{1}-{2}-{3}.bmp'.format(
                                              epoch, idx, i, name),
                                          std=self.config.std,
                                          mean=self.config.mean)
                        utils.save_images(watermarked_images=self.train_cover[i].cpu(),
                                          filename='./DeepSteg/cover/epoch-{0}-cover-batch-{1}-{2}-{3}.bmp'.format(
                                              epoch, idx, i, name),
                                          std=self.config.std,
                                          mean=self.config.mean)
                        # utils.save_images(watermarked_images=attacked[i].cpu(),
                        #                   filename='./DeepSteg/attacked/epoch-{0}-attacked-batch-{1}-{2}-{3}.bmp'.format(
                        #                       epoch, idx, i, name),
                        #                   std=self.config.std,
                        #                   mean=self.config.mean)
                        utils.save_images(watermarked_images=Residual[i].cpu(),
                                          filename='./DeepSteg/residual/epoch-{0}-residual-batch-{1}-{2}-{3}.bmp'.format(
                                              epoch, idx, i, name))
                    ...
                    if self.config.conductTest:
                        ...
                        for attack_num in range(len(self.net.noise_layers)):
                            ...
                            utils.save_images(watermarked_images=marked[0].cpu(),
                                              filename='./DeepSteg/Test/marked/epoch-{0}-marked-batch-{1}-{2}-{3}.bmp'.format(
                                                  epoch, idx, 0, name),
                                              std=self.config.std,
                                              mean=self.config.mean)
                            utils.save_images(watermarked_images=extracted[0].cpu(),
                                              filename='./DeepSteg/Test/extracted/epoch-{0}-extracted-batch-{1}-{2}-{3}.bmp'.format(
                                                  epoch, idx, 0, name),
                                              std=self.config.std,
                                              mean=self.config.mean)
                            utils.save_images(watermarked_images=self.test_water[0].cpu(),
                                              filename='./DeepSteg/Test/water/epoch-{0}-water-batch-{1}-{2}-{3}.bmp'.format(
                                                  epoch, idx, 0, name),
                                              std=self.config.std,
                                              mean=self.config.mean)
                            utils.save_images(watermarked_images=self.test_cover[0].cpu(),
                                              filename='./DeepSteg/Test/cover/epoch-{0}-cover-batch-{1}-{2}-{3}.bmp'.format(
                                                  epoch, idx, 0, name),
                                              std=self.config.std,
                                              mean=self.config.mean)
                            utils.save_images(watermarked_images=attacked[0].cpu(),
                                              filename='./DeepSteg/Test/attacked/epoch-{0}-attacked-batch-{1}-{2}-{3}.bmp'.format(
                                                  epoch, idx, 0, name),
                                              std=self.config.std,
                                              mean=self.config.mean)
                            utils.save_images(watermarked_images=Residual[0].cpu(),
                                              filename='./DeepSteg/Test/residual/epoch-{0}-residual-batch-{1}-{2}-{3}.bmp'.format(
                                                  epoch, idx, 0, name))
...
if __name__ == '__main__':
    folders = ['./DeepSteg/','./DeepSteg/cover/','./DeepSteg/extracted/','./DeepSteg/marked/','./DeepSteg/residual/','./DeepSteg/water/','./DeepSteg/attacked/']
